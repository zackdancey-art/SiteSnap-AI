...
- **In-memory mode:** `src/test-setup.ts` (the `--require` preload) already does this for every test — it sets `NODE_ENV=test`, installs a test `AUTH_TOKEN_SECRET`, and blanks **every** environment variable outside a short explicit **allowlist**. You do not need per-file reset lines, and a per-file `delete process.env.X` is actively wrong: `server.ts` calls `dotenv.config()` on import, and dotenv repopulates an *absent* key while leaving an *empty* one alone — so assign `""`, never `delete`. If a test needs a variable the allowlist blanks, set it explicitly in the test file, or add it to the allowlist with a reason. `TEST_ENV_DEBUG=1` prints what was blanked. **Consequence:** the in-memory suite cannot exercise anything Postgres-only — NOT NULL constraints, foreign keys, or Row-Level Security. Tests that must prove RLS or a DB constraint have to run against a real Postgres via `TEST_DATABASE_URL` and skip when it is unset (this environment has no local Postgres).
- **Every negative assertion needs a positive control in the same test.** `length === 0`, `=== null`, `doesNotMatch`, `!list.includes(x)` — each passes both when the code ran and correctly produced nothing *and* when the code never ran at all. So alongside "B must not see A's site", assert "A **does** see A's site" against the same endpoint. This is not pedantry: mutating `/uploads/sign` to deny **every** caller left all 126 tests green, because the only assertion on it was a negative one (`docs/VACUITY-AUDIT.md`, M7). Same rule for setup: use the `seed()` wrapper, never a bare `await req(...)` whose response is discarded — a seed that starts failing turns every assertion below it into a test of nothing.
...
- **No external calls in tests:** never hit real Twilio, real S3, real OpenAI, or any network. Mock at the boundary. The allowlist in `test-setup.ts` is what now makes this structural rather than aspirational — before it, `.env` supplied live RESEND/TWILIO/S3/SENTRY credentials to the suite, and because `.env` also sets `MEDIA_STORAGE_PROVIDER=s3` (and `useS3Storage()` has no `NODE_ENV` guard) the upload tests were writing to the **real bucket** and passing because the write succeeded. (Mocking SMS/email at the boundary is finding **H3a**; until it lands, be aware the pre-commit hook can fail on Twilio quota.)
...
- **What actually gates a commit vs. a merge.** The pre-commit hook (`.git/hooks/pre-commit`) is **branch-aware**: on feature branches it runs **`typecheck` + `lint` only** (tests are kept out to keep the commit loop fast); on `main`/`master` it additionally runs `test`. So on a feature branch a `--no-verify` is justified only when an intermediate logical-split commit doesn't individually pass **typecheck/lint** — tests are *not* the blocker there, and the earlier belief that "the hook's full-suite check can't pass on intermediate commits" was false. The **full suite is gated by CI, not the hook**: [`.github/workflows/ci.yml`](.github/workflows/ci.yml) runs `lint` + `typecheck` + **`test` (in-memory)** + **`test:db` (DB-gated suites against a `postgres:16` service container)** on **every `pull_request`** (and on push to `main`/`master`). The DB step is what makes the RLS + store round-trip suites actually run rather than skip; if a PR's "Tests (database)" step reports skips or a connection error, that's a CI service-wiring problem to fix before merge, not a pass. Treat green CI on the PR as the real pre-merge gate; the branch tip must pass typecheck + lint locally and the full suite in CI before merge.
